...
BackendEngine::BackendEngine(SharedQueue& queue, const std::string& state_path, bool standalone)
    : queue_(queue), state_path_(state_path), file_lock_(state_path)
{
    load_state();
    if (standalone) is_writer_ = true;
    else try_acquire_write_lock();
}
...
void BackendEngine::try_acquire_write_lock() {
    is_writer_ = file_lock_.acquire_exclusive();
    if (is_writer_) last_file_mtime_ = Platform::current_time_ms();
}

void BackendEngine::check_and_reload_if_changed() {
    if (is_writer_) return;
    uint64_t mtime = Platform::file_last_write_time_ms(state_path_);
    if (mtime > last_file_mtime_) { last_file_mtime_ = mtime; load_state(); }
}

void BackendEngine::load_state() {
    StateFile sf(state_path_);
    if (sf.exists()) sf.load(user_mgr_, users_);
    // 重新设置 mutex（loaded schedulers 有 nullptr mutex）
...
void BackendEngine::save_state() {
    ...
    last_file_mtime_ = Platform::current_time_ms();
}
...
std::string BackendEngine::overview(const std::string& username) {
    auto& u = get_or_create_user(username);
    std::ostringstream oss;
    oss << "╔══════════════════ System Overview ══════════════════╗" << std::endl;
    oss << u.pm.ptree() << std::endl;
    oss << std::endl;
    oss << u.mm.show_mem() << std::endl;
    oss << std::endl;
...
static bool is_write_cmd(MessageType t) {
    switch (t) {
        ...
        case MessageType::CREATE_PCB: case MessageType::KILL_PCB:
        case MessageType::BLOCK_PCB: case MessageType::WAKEUP_PCB:
        case MessageType::SUSPEND: case MessageType::RESUME:
...
std::string BackendEngine::dispatch(const Message& msg, const std::string& username) {
    ...
    if (is_write_cmd(msg.type) && !is_writer_)
        return "Error: 只读模式。";
    ...
    switch (msg.type) {
        ...
        case MessageType::CREATE_PCB: {
            uint32_t req_time = 0, parent = 1; int ql = 0;
            try {
                req_time = std::stoul(arg(msg.args, 1));
            ...
            if (ql < 0 || ql > 2) return "Error: 队列层级为 0/1/2。";
            auto result = u.pm.create_pcb(arg(msg.args, 0), req_time, username, ql, parent);
            auto lp = result.find('('), rp = result.find(',', lp);
            if (lp != std::string::npos && rp != std::string::npos) {
                try {
                    uint32_t pid = std::stoul(result.substr(lp+1, rp-lp-1));
        ...
        case MessageType::KILL_PCB: {
            ...
            for (auto rp : to_remove) { u.mlfq.remove(rp); u.mm.free_all_by_pid(rp); }
        ...
        case MessageType::WAKEUP_PCB: {
            ...
            if (pcb->state == ProcessState::BLOCKED && pcb->swapped_out) {
                uint32_t addr = u.mm.pgfault_alloc(pid, pcb->swapped_size);
                if (addr != UINT32_MAX) {
                    ...
                    return pcb->name + "(" + std::to_string(pid)
                           + ") 唤醒失败: 内存不足(" + std::to_string(pcb->swapped_size) + "KB)，仍阻塞。";
                }
            }
            auto result = u.pm.wakeup_pcb(pid);
            if (result.find("已唤醒") != std::string::npos)
                if (pcb && !pcb->suspended) u.mlfq.enqueue_to_level(pid, pcb->queue_level);
            return result;
        }
        ...
        case MessageType::RESUME: {
            uint32_t pid = 0;
            try { pid = std::stoul(arg(msg.args, 0)); } catch (...) { return "Error: PID 无效。"; }
            auto result = u.pm.resume(pid);
            if (result.find("已恢复") != std::string::npos) {
                PCB* pcb = u.pm.get_pcb(pid);
        ...
        case MessageType::ALLOC: {
            ...
            auto result = u.mm.alloc(size, pid, pcb->name);
            if (result.find("分配成功") != std::string::npos) {
                auto pos = result.find("起始地址="), end = result.find("KB", pos);
                if (pos != std::string::npos && end != std::string::npos) {
                    try {
                        uint32_t addr = std::stoul(result.substr(pos+13, end-pos-13));
        ...
        case MessageType::COMPACT: {
            auto mapping = u.mm.compact();
            for (const auto& cr : mapping) {
            ...
            std::ostringstream oss;
            oss << "compact 完成。" << mapping.size() << " 个块前移。";
            return oss.str();
        }
        ...
        case MessageType::SWAP_OUT: {
            uint32_t pid = 0;
            try { pid = std::stoul(arg(msg.args, 0)); } catch (...) { return "Error: PID 无效。"; }
            auto [size, result] = u.mm.swap_out(pid);
            if (size > 0) {
                PCB* pcb = u.pm.get_pcb(pid);
                if (pcb) {
                    pcb->swapped_out = true; pcb->swapped_size = size;
                    pcb->mem_block_addrs.clear(); pcb->mem_block_sizes.clear();
                }
            }
            return result;
        ...
        case MessageType::SAVE: {
            save_state();
            return "状态已保存到 " + state_path_;
        }
        case MessageType::LOAD: { load_state(); return "状态已加载。"; }
        case MessageType::OVERVIEW: return overview(username);
...
std::string BackendEngine::process(const Message& msg, const std::string& username) {
    ...
    if (is_write_cmd(msg.type)) {
        bool expected = false;
        if (!write_busy_.compare_exchange_strong(expected, true))
            return "Error: 另一个客户端正在执行写操作，请稍后再试。";
    }
    ...
    if (is_write_cmd(msg.type) && msg.type != MessageType::SAVE)
        save_state();

    if (is_write_cmd(msg.type)) write_busy_.store(false);
    return result;
}

void BackendEngine::run() {
    std::string session_user;
    while (!queue_.is_shutdown()) {
        uint32_t timeout = is_writer_ ? 1000U : SCHED_POLL_INTERVAL_MS;
        std::optional<Message> msg_opt;
        try { msg_opt = queue_.try_pop_for(timeout); }
        catch (...) { return; }
        if (!msg_opt.has_value()) { check_and_reload_if_changed(); continue; }

        Message& msg = msg_opt.value();
        if (msg.type == MessageType::LOGIN) {
            auto r = user_mgr_.login(arg(msg.args, 0), arg(msg.args, 1));
            if (r.find("登录成功") != std::string::npos) session_user = arg(msg.args, 0);
            try { msg.promise.set_value(r); } catch (...) {}
            continue;
        }
        if (msg.type == MessageType::LOGOUT) {
            session_user.clear();
            try { msg.promise.set_value("已登出。"); } catch (...) {}
            continue;
        }

        std::string result;
        try { result = dispatch(msg, session_user); }
        catch (...) { result = "Error: 内部错误。"; }
        try { msg.promise.set_value(result); } catch (...) {}
    }
}
